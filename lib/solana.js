...
export const PROGRAM_ID = new PublicKey(
  "GqZPaDR3Mx3eL75VGeRErRGiDB8cDRtwa66dYRQCt5fY"
);
...
export async function deriveV2ProgramStatePda() {
  return PublicKey.findProgramAddress(
    [threadIdBytes("program_state_v2")],
    PROGRAM_ID,
  );
}

export async function deriveV2SharedVaultPda() {
  return PublicKey.findProgramAddress(
    [threadIdBytes("shared_vault_v2")],
    PROGRAM_ID,
  );
}

export async function deriveV2FeeVaultPda() {
  return PublicKey.findProgramAddress(
    [threadIdBytes("fee_vault_v2")],
    PROGRAM_ID,
  );
}

export async function deriveRoomLedgerPda(threadId) {
  return PublicKey.findProgramAddress(
    [threadIdBytes("room_ledger"), await roomSeedBytes(threadId)],
    PROGRAM_ID,
  );
}

export async function deriveRoomReceiptPda(threadId, userPubkey) {
  return PublicKey.findProgramAddress(
    [threadIdBytes("room_receipt"), await roomSeedBytes(threadId), toPublicKey(userPubkey).toBuffer()],
    PROGRAM_ID,
  );
}

export async function deriveV2CurvePda(threadId) {
  return PublicKey.findProgramAddress(
    [threadIdBytes("v2_curve"), await roomSeedBytes(threadId)],
    PROGRAM_ID,
  );
}

export async function deriveV2CurveAuthorityPda(threadId) {
  return PublicKey.findProgramAddress(
    [threadIdBytes("v2_curve_authority"), await roomSeedBytes(threadId)],
    PROGRAM_ID,
  );
}

export async function deriveV2SpawnPoolPda(threadId) {
  return PublicKey.findProgramAddress(
    [threadIdBytes("v2_spawn_pool"), await roomSeedBytes(threadId)],
    PROGRAM_ID,
  );
}

export async function deriveV2MintPda(threadId) {
  return PublicKey.findProgramAddress(
    [threadIdBytes("v2_mint"), await roomSeedBytes(threadId)],
    PROGRAM_ID,
  );
}

export async function deriveV2CurveTokenVaultPda(threadId) {
  return PublicKey.findProgramAddress(
    [threadIdBytes("v2_curve_token_vault"), await roomSeedBytes(threadId)],
    PROGRAM_ID,
  );
}

...
const DEFAULT_DEVNET_RPC = clusterApiUrl(SOLANA_CLUSTER);

function resolveRpcEndpoint(){
  const globalRpc = typeof globalThis !== "undefined"
    ? String(globalThis.PINGY_RPC_URL || globalThis.__PINGY_RPC_URL || "").trim()
    : "";
  if(globalRpc) return { rpcUrl: globalRpc, source: "global" };

  const queryRpc = typeof window !== "undefined"
    ? String(new URLSearchParams(window.location.search).get("rpc") || "").trim()
    : "";
  if(queryRpc) return { rpcUrl: queryRpc, source: "query" };

  return { rpcUrl: DEFAULT_DEVNET_RPC, source: "default" };
}

export const RPC_ENDPOINT_RESOLUTION = resolveRpcEndpoint();
export const DEVNET_RPC = RPC_ENDPOINT_RESOLUTION.rpcUrl;

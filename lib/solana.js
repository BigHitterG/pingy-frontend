...
// Guard invalid base58 to keep the UI bootable even when local config drifts.
const PROGRAM_ID_FALLBACK = "Vote111111111111111111111111111111111111111";
const PROGRAM_ID_RAW = "Pingy1111111111111111111111111111111111111";

let programId = null;
try {
  programId = new PublicKey(PROGRAM_ID_RAW);
} catch (err) {
  console.warn(
    "[pingy] invalid PROGRAM_ID in lib/solana.js, falling back to",
    PROGRAM_ID_FALLBACK,
    err,
  );
  programId = new PublicKey(PROGRAM_ID_FALLBACK);
}

function createMockSolanaWeb3(){
  ...
  const mockProgramId = new MockPublicKey("11111111111111111111111111111111");
  ...
  return {
    ...
    SystemProgram: {
      ...
      transfer({ fromPubkey, toPubkey, lamports = 0 }){
        return new MockTransactionInstruction({
          ...
          data: new Uint8Array([Number(lamports) & 0xff]),
        });
      },
    },
...
const globalWeb3 = typeof window !== "undefined" ? window.solanaWeb3 : null;
const solanaWeb3 = globalWeb3 || createMockSolanaWeb3();

if(!globalWeb3){
  console.warn("[pingy] using mock solana client (no browser solanaWeb3 detected)");
}

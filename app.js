...
const $ = (id) => document.getElementById(id);
    ...
    function applyLaunchBackendDefaults(room, { creatorCommitSol = null } = {}){
      if(!room) return room;
      ...
      if(isPumpfunLaunchBackend()){
        ...
        room.external_launch_url = String(room.external_launch_url || "");
        room.external_mint = String(room.external_mint || "");
        room.funding_mode = room.funding_mode || "vault";
        room.creator_commit_sol = Number.isFinite(parsedCommit) && parsedCommit > 0 ? parsedCommit : 0;
      } else {
        ...
        room.external_mint = String(room.external_mint || "");
        room.funding_mode = room.funding_mode || "vault";
        room.creator_commit_sol = Number.isFinite(Number(room.creator_commit_sol)) ? Number(room.creator_commit_sol) : 0;
      }
      return room;
    }
...
function encodeU64Arg(v){
  ...
  return out;
    ...
    state.rooms.forEach((r) => {
      ...
      applyLaunchBackendDefaults(r, { creatorCommitSol: r.creator_commit_sol });
    ...
    function mkRoom(id, name, ticker, desc, launchConfig = null, options = null){
      ...
      const useNativeInstantState = isInstant && isNativeLaunchBackend();
      const room = {
        id, name, ticker, desc,
        creator_wallet,
        socials: { x:'', tg:'', web:'' },
        created_at: nowStamp(),
        state: useNativeInstantState ? "BONDING" : "SPAWNING",          // SPAWNING | BONDING | BONDED
      ...
      return applyLaunchBackendDefaults(room, { creatorCommitSol: options?.creatorCommitSol });
    }
...
if(connectBtn){
  connectBtn.addEventListener("click", () => {
    ...
    async function createCoinFromForm(){
      ...
      if(isNativeLaunchBackend() && shouldUseOnchain()){
      ...
      const r = mkRoom(id, name, ticker, desc, launchConfig, { creatorCommitSol: commit });
      ...
      if(isNativeLaunchBackend() && launchMode === "instant"){
      ...
      const createMessage = isPumpfunLaunchBackend()
        ? (launchMode === "instant" ? "coin created. ready for external launch." : "coin created. waiting for spawn.")
        : (launchMode === "instant" ? "coin created. launched coins will trade externally." : "coin created. waiting for spawn.");
      state.chat[id] = [{ ts:"—", wallet:"SYSTEM", text: createMessage }];
